...
export { ICON_HEART, ICON_FLAG, ICON_COMMUNITY, ICON_RUN };
...
export function decomposeTechnique(key: string): TechComponent[] {
  ...
  if (key === 'left_right' || key.startsWith('both'))
...
const MOVEMENT_LABELS: Record<string, string> = {
  running: 'Running',
  walking: 'Walking',
  crouched_walking: 'Crouch Walk',
  crouched_stationary: 'Crouched',
};

export function movementComponent(movement: string): TechComponent | null {
  if (!movement || movement === 'stationary') return null;
  return { icon: ICON_RUN, label: MOVEMENT_LABELS[movement] || movement };
}
